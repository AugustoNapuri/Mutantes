...
public class AdnSequenceUtil extends MutantesApplicationTests {

    @Test
    public void getColumnsSequences() {
        String[] columns = adnSequenceUtil.extractColumns(new String[]{"0123", "1111", "2222", "3333"});
    ...
    public void getDiagonalsSequences() {
        String[] diagonals = adnSequenceUtil.extractDiagonals(new String[]{"0XX7", "X16X", "X52X", "4XX3"});
        assertEquals("0123", diagonals[0]);
        assertEquals("4567", diagonals[1]);

        String row = "XXXXX";
        diagonals = adnSequenceUtil.extractDiagonals(new String[]{row, row, row, row, row});
    ...
    public void checkValidityTest() {
        ...
        assertFalse(adnSequenceUtil.checkValidity(wrongSequence));
        assertTrue(adnSequenceUtil.checkValidity(validSequence));
    }
}
